...
function CreateProductForm() {
  ...
  // Actualizar la sección cuando se hace clic en la barra lateral
  const handleSetSection = (section) => {
    if (section === "Products") {
      navigate("/admin"); // Redirige al dashboard de productos
    }
    if (section === "Users") {
      navigate("/admin/users"); // Agrega rutas adicionales si es necesario
    }
    if (section === "Carts") {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage(
          `Error: ${errorData.message || "No se pudo crear el producto"}`
        );
      }
    } catch (error) {
  ...
  return (
    <AdminLayout setSection={handleSetSection}>
